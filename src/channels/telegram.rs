...
pub struct TelegramChannel {
    ...
    /// In-memory cache of seen Telegram users for state file tracking.
    seen_telegram_users: Mutex<std::collections::HashMap<String, SeenTelegramUser>>,
}

/// Tracked Telegram user info for the state file.
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
struct SeenTelegramUser {
    telegram_id: String,
    chat_id: String,
    last_seen: String,
}
...
impl TelegramChannel {
    pub fn new(bot_token: String, allowed_users: Vec<String>, mention_only: bool) -> Self {
        ...
        Self {
            ...
            workspace_dir: None,
            seen_telegram_users: Mutex::new(std::collections::HashMap::new()),
        }
    }

    /// Configure workspace directory for saving downloaded attachments.
    /// Also loads any previously saved telegram_users.json into memory.
    pub fn with_workspace_dir(mut self, dir: std::path::PathBuf) -> Self {
        // Load existing seen users from disk so we don't lose them on restart
        let state_path = dir.join("state/telegram_users.json");
        if let Ok(content) = std::fs::read_to_string(&state_path) {
            if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(&content) {
                if let Some(users) = parsed.get("users").and_then(|u| u.as_object()) {
                    let mut map = self.seen_telegram_users.lock();
                    for (key, info) in users {
                        if let (Some(tid), Some(cid)) = (
                            info.get("telegram_id").and_then(|v| v.as_str()),
                            info.get("chat_id").and_then(|v| v.as_str()),
                        ) {
                            let last_seen = info
                                .get("last_seen")
                                .and_then(|v| v.as_str())
    ...
    /// Track a seen Telegram user and write state file if new/changed.
    /// Called from the listen loop after a message is successfully parsed.
    fn track_seen_user(&self, update: &serde_json::Value) {
        let message = update
            .get("message")
        ...
        // For DMs, chat_id == sender's numeric ID.
        // For groups, we still record the sender's numeric ID as chat_id
        // since proactive delivery targets the user's DM.
        let chat_id = sender_id.clone();

        // Use username as key if available, otherwise numeric ID
        let key = if username != "unknown" {
            username.clone()
        } else {
            sender_id.clone()
        };
        ...
        let is_new = {
            let mut seen = self.seen_telegram_users.lock();
            let entry = seen.get(&key);
            let changed = match entry {
                None => true,
                Some(existing) => existing.telegram_id != sender_id || existing.chat_id != chat_id,
            };
            if changed {
                seen.insert(
                    key,
        ...
        if is_new {
            self.flush_seen_users_to_disk();
        }
    }

    /// Write the seen users map to disk as telegram_users.json.
    fn flush_seen_users_to_disk(&self) {
        let workspace = match self.workspace_dir.as_ref() {
            Some(w) => w,
            None => return,
        };
        ...
        match serde_json::to_string_pretty(&payload) {
            Ok(json) => {
                if let Err(e) = std::fs::write(&path, json) {
                    tracing::warn!("Failed to write telegram_users.json: {}", e);
                }
            }
            Err(e) => {
                tracing::warn!("Failed to serialize telegram users: {}", e);
            }
        }
    }
...
Ensure only one `zeroclaw` process is using this bot token."
            ...
            if let Some(results) = data.get("result").and_then(serde_json::Value::as_array) {
                for update in results {
                    ...
                    // Track seen user for proactive delivery target discovery
                    self.track_seen_user(update);
