...
fn build_recent_run_context(config: &Config, job_id: &str) -> String {
    ...
    // Filter to recent successful runs with real output (not NO_REPLY),
    // within the last 48 hours — take up to 3.
    ...
    for run in &relevant {
        ...
        let age = format_run_age(now, run.finished_at);
        let truncated = if output.len() > RUN_HISTORY_MAX_OUTPUT_CHARS {
            // Find a safe char boundary for truncation
            let mut cutoff = RUN_HISTORY_MAX_OUTPUT_CHARS;
            while cutoff > 0 && !output.is_char_boundary(cutoff) {
                cutoff -= 1;
            }
            format!("{}...", &output[..cutoff])
        } else {
            output.to_string()
        };
    ...
    ctx.push_str("=== END PREVIOUS — only report NEW information not already covered above ===\n");
    ctx
}

fn format_run_age(now: DateTime<Utc>, at: DateTime<Utc>) -> String {
    let mins = (now - at).num_minutes();
    if mins < 60 {
        format!("{mins} min ago")
    } else {
        let hours = mins / 60;
...
async fn run_agent_job(
    ...
    // Inject recent run history so the agent knows what it already did
    // and doesn't repeat topics/tasks from previous runs.
    let history = build_recent_run_context(config, &job.id);
